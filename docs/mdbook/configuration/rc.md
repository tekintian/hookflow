...
- Or even if your GUI program cannot locate the `hookflow` executable :scream:
- Or if you want to use ENV variables that control the executables behavior in `hookflow.yml`
...
# You can share it between projects where you use hookflow.
...
rc: '"${XDG_CONFIG_HOME:-$HOME/.config}/hookflowrc"'
```
