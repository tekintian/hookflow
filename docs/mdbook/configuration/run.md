...
- `{cmd}` - shorthand for the command from `hookflow.yml`.
...
- `{hookflow_job_name}` - current job/command/script name

> **Note:** Command line length has a limit on every system. If your list of files is quite long, hookflow splits your files list to fit in the limit and runs few commands sequentially.
