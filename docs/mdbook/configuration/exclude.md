...
If you've specified `exclude` but don't have a files template in [`run`](./run.md) option, hookflow will check `{staged_files}` for `pre-commit` hook and `{push_files}` for `pre-push` hook and apply filtering. If no files left, the command will be skipped.
