...
1. Run `hookflow add -d pre-commit`
1. Edit `.hookflow/pre-commit/my-script.sh`
1. Add an entry to `hookflow.yml`
   ```yml
   # hookflow.yml
...
Let's create a bash script to check commit templates `.hookflow/commit-msg/template_checker`:
...
Now we can ask hookflow to run our bash script by adding this code to
`hookflow.yml` file:
