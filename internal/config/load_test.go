...
`,
		...
		"with hookflow.yml and .hookflow.yml": {
			files: map[string]string{
				".hookflow.yml": `
pre-commit:
  commands:
...
`,
		...
		"with overrides from .hookflow-local.yml": {
			files: map[string]string{
				".hookflow.yml": `
pre-push:
  scripts:
...
pre-commit:
  ...
  scripts:
    "test.sh":
      runner: bash
`,
			remoteConfigPath: filepath.Join(root, ".git", "info", "hookflow-remotes", "hookflow", "hookflow.yml"),
			result: &Config{
				SourceDir:      DefaultSourceDir,
				SourceDirLocal: DefaultSourceDirLocal,
				Colors:         nil,
				Remotes: []*Remote{
					{
						GitURL: "git@github.com:tekintian/hookflow",
					},
				},
				Hooks: map[string]*Hook{
					"pre-commit": {
...
pre-commit:
  ...
  scripts:
    "test.sh":
      ...
      runner: bash
`,
			remoteConfigPath: filepath.Join(root, ".git", "info", "hookflow-remotes", "hookflow-v1.0.0", "examples", "custom.yml"),
			result: &Config{
				SourceDir:      DefaultSourceDir,
				SourceDirLocal: DefaultSourceDirLocal,
				Colors:         nil,
...
pre-push:
  scripts:
    "local-extend":
      runner: bash
`,
				".git/info/hookflow-remotes/hookflow/remote-extend.yml": `
pre-push:
  scripts:
    "remote-extend":
      runner: bash
...
pre-push:
  commands:
    remote:
      run: echo remote
`,
			remoteConfigPath: filepath.Join(root, ".git", "info", "hookflow-remotes", "hookflow", "examples", "config.yml"),
			result: &Config{
				SourceDir:      DefaultSourceDir,
				SourceDirLocal: DefaultSourceDirLocal,
				Colors:         nil,
				Remotes: []*Remote{
					{
						GitURL:  "https://github.com/tekintian/hookflow",
...
`,
		...
		"with .config/hookflow.yml": {
			files: map[string]string{
				filepath.Join(".config", "hookflow.yml"): `
pre-commit:
  jobs:
    - name: job 1
      run: echo from job 1
`,
				filepath.Join(".config", "hookflow-local.yml"): `
pre-commit:
  jobs:
    - name: job 1
      run: wrap {cmd}
...
`,
			...
			pathOverride: "hookflow_custom.yml",
			result: &Config{
				SourceDir:      DefaultSourceDir,
				SourceDirLocal: DefaultSourceDirLocal,
				Colors:         nil,
		...
		t.Run(name, func(t *testing.T) {
			...
			t.Setenv("HOOKFLOW_CONFIG", tt.pathOverride)
...
`,
		...
		t.Run(fmt.Sprintf("%d: %s", i, tt.name), func(t *testing.T) {
			...
			// YAML
			yamlConfig := filepath.Join(root, "hookflow.yml")
			...
			// JSON
			jsonConfig := filepath.Join(root, "hookflow.json")
			...
			// TOML
			tomlConfig := filepath.Join(root, "hookflow.toml")
...
`,
				...
				{
					RemoteConfigPath: filepath.Join(root, ".git", "info", "hookflow-remotes", "hookflow-v1.5.5", "examples", "remote", "ping.yml"),
					Content: `
pre-commit:
  commands:
    ping:
