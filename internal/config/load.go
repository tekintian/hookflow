...
const (
	DefaultConfigName     = "hookflow.yml"
	DefaultSourceDir      = ".hookflow"
	DefaultSourceDirLocal = ".hookflow-local"
)

var (
	hookKeyRegexp    = regexp.MustCompile(`^(?P<hookName>[^.]+)\.(?:scripts|commands|jobs)`)
	LocalConfigNames = []string{"hookflow-local", ".hookflow-local", filepath.Join(".config", "hookflow-local")}
	MainConfigNames  = []string{"hookflow", ".hookflow", filepath.Join(".config", "hookflow")}
	extensions       = []string{
		".yml",
		".yaml",
		".json",
...
// loadFirstMain loads the main config (e.g. hookflow.yml) or fallbacks to local config (e.g. hookflow-local.yml).
...
func LoadSecondary(main *koanf.Koanf, filesystem afero.Fs, repo *git.Repository) (*koanf.Koanf, error) {
	...
	// Don't allow to set `hookflow` field from a remote config
	secondary.Delete("hookflow")

	// Load optional local config (e.g. hookflow-local.yml)
	var noLocal bool
...
func LoadKoanf(filesystem afero.Fs, repo *git.Repository) (*koanf.Koanf, *koanf.Koanf, error) {
	// Load main hookflow.yml
	main, err := loadMain(filesystem, repo.RootPath)
	if err != nil {
		return nil, nil, err
	}

	// Load secondary extends, remotes and hookflow-local.yml
...
func addHook(name string, main, secondary *koanf.Koanf, c *Config) error {
	...
	if tags := os.Getenv("HOOKFLOW_EXCLUDE"); tags != "" {
