...
const (
	EnvVerbose             = "HOOKFLOW_VERBOSE" // keep all output
	envNoColor             = "NO_COLOR"
	envForceColor          = "CLICOLOR_FORCE"
	hookFileMode           = 0o755
	oldHookPostfix         = ".old"
	hookContentFingerprint = "HOOKFLOW"
)
...
// NewHookflow returns an instance of Hookflow.
func NewHookflow(verbose bool, colors string) (*Hookflow, error) {
	fs := afero.NewOsFs()
	...
	return &Hookflow{fs: fs, repo: repo}, nil
}

func (l *Hookflow) LoadConfig() (*config.Config, error) {
	return config.Load(l.fs, l.repo)
}

func (l *Hookflow) reloadConfig(cfg *config.Config) (*config.Config, error) {
...
// Tests a file whether it is a hookflow-created file.
func (l *Hookflow) isHookflowFile(path string) bool {
...
// Removes the hook from hooks path, saving non-hookflow hooks with .old suffix.
func (l *Hookflow) cleanHook(hook string, force bool) error {
	...
	// Just remove hookflow hook
	if l.isHookflowFile(hookPath) {
...
func (l *Hookflow) addHook(hook string, args templates.Args) error {
	hookPath := filepath.Join(l.repo.HooksPath, hook)
