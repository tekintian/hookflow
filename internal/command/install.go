...
var (
	hookflowChecksumRegexp = regexp.MustCompile(`(\w+)\s+(\d+)(?:\s+([\w,-]+))?`)
	errNoConfig            = errors.New("no hookflow config found")
)
...
func (l *Hookflow) Install(ctx context.Context, args InstallArgs, hooks []string) error {
...
func (l *Hookflow) readOrCreateConfig() (*config.Config, error) {
...
func (l *Hookflow) configExists(path string) bool {
	configPath, _ := l.findMainConfig(path)
	return configPath != ""
}

func (l *Hookflow) findMainConfig(path string) (string, error) {
	configOverride := os.Getenv("HOOKFLOW_CONFIG")
	if len(configOverride) != 0 {
		...
		if ok, _ := afero.Exists(l.fs, configOverride); !ok {
			return "", fmt.Errorf("couldn't find config from HOOKFLOW_CONFIG: %s", configOverride)
		}
...
func (l *Hookflow) createConfig(path string) error {
...
func (l *Hookflow) syncHooks(cfg *config.Config, fetchRemotes bool) (*config.Config, error) {
	var remotesSynced bool
...
func (l *Hookflow) shouldRefetch(remote *config.Remote) bool {
...
func (l *Hookflow) findAvailableRemoteRef(url string) (string, error) {
	entries, err := afero.ReadDir(l.fs, l.repo.RemotesFolder())
	if err != nil {
		return "", err
	}
...
func (l *Hookflow) createHooksIfNeeded(cfg *config.Config, hooks []string, force bool) error {
	...
	for hook := range cfg.Hooks {
		...
		templateArgs := templates.Args{
			Rc:                      cfg.Rc,
			AssertHookflowInstalled: cfg.AssertHookflowInstalled,
			Roots:                   roots,
			HookflowPath:            cfg.Hookflow,
		}
...
func (l *Hookflow) checkHooksSynchronized(cfg *config.Config) (bool, []string) {
	...
	for scanner.Scan() {
		match := hookflowChecksumRegexp.FindStringSubmatch(scanner.Text())
		if match != nil {
...
func (l *Hookflow) configLastUpdateTimestamp() (int64, error) {
...
func (l *Hookflow) addChecksumFile(checksum string, hooks []string) error {
...
func (l *Hookflow) checksumFilePath() string {
	return filepath.Join(l.repo.InfoPath, config.ChecksumFileName)
}

func (l *Hookflow) ensureHooksDirExists() error {
