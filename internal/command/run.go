...
const (
	envEnabled = "HOOKFLOW"        // "0", "false"
	envOutput  = "HOOKFLOW_OUTPUT" // "meta,success,failure,summary,skips,execution,execution_out,execution_info"
)
...
func (l *Hookflow) Run(ctx context.Context, args RunArgs) error {
	...
	// See: internal/hookflow/install.go
	...
	if !args.NoAutoInstall && !cfg.NoAutoInstall {
		...
		if err != nil {
			log.Warnf(
				"⚠️  There was a problem with synchronizing git hooks. Run 'hookflow install' manually.\n   Error: %s", err,
			)
		} else {
			cfg = newCfg
		}
...
func resolveHook(cfg *config.Config, hookName string) (*config.Hook, error) {
	...
	if !ok {
		if config.KnownHook(hookName) {
			log.Debugf("[hookflow] skip: Hook %s doesn't exist in the config", hookName)
			return nil, nil
		}
...
func getSourceDirs(repo *git.Repository, cfg *config.Config) []string {
	sourceDirs := []string{
		...
		filepath.Join(repo.RootPath, ".config", "hookflow"),
		filepath.Join(repo.RootPath, ".config", "hookflow-local"),
	}
...
func checkVersion(minVersion string) error {
	...
	if err := version.Check(minVersion, version.Version(false)); err != nil {
		...
		return fmt.Errorf("required hookflow version (%s) is higher than current (%s) at %s", minVersion, version.Version(false), execPath)
	}
