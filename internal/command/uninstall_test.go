...
func TestHookflowUninstall(t *testing.T) {
	...
	for n, tt := range [...]struct {
	...
	}{
		{
			...
			existingHooks: map[string]string{
				"pre-commit":  "not a hookflow hook",
		...
		},
		{
			...
			existingHooks: map[string]string{
				"pre-commit":      "not a hookflow hook",
				"post-commit":     "HOOKFLOW file",
				"post-commit.old": "not a hookflow hook",
			},
			config: "# empty",
			wantExist: []string{
				configPath,
		...
		t.Run(fmt.Sprintf("%d: %s", n, tt.name), func(t *testing.T) {
			...
			// Do uninstall
			err = hookflow.Uninstall(t.Context(), tt.args)
			if err != nil {
				t.Errorf("unexpected error: %s", err)
			}
