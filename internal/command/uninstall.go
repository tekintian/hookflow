...
func (l *Hookflow) Uninstall(_ctx context.Context, args UninstallArgs) error {
...
func (l *Hookflow) deleteHooks(force bool) error {
	...
	for _, file := range hooks {
		...
		// Skip non-hookflow files if removal not forced
		if !l.isHookflowFile(hookFile) && !force {
			continue
		}
...
func (l *Hookflow) removeFile(glob string) {
