// Package updater contains the self-update implementation for the hookflow executable.
package updater
...
const (
	...
	latestReleaseURL              = "https://api.github.com/repos/tekintian/hookflow/releases/latest"
	checksumsFilename             = "hookflow_checksums.txt"
	checksumFields                = 2
	modExecutable     os.FileMode = 0o755
)

var (
	errNoAsset        = errors.New("couldn't find an asset to download. Please submit an issue to https://github.com/tekintian/hookflow")
	errInvalidHashsum = errors.New("SHA256 sums differ, it's not safe to use the downloaded binary.\nIf you have problems upgrading hookflow please submit an issue to https://github.com/tekintian/hookflow")
...
func (u *Updater) SelfUpdate(ctx context.Context, opts Options) error {
	...
	wantedAsset := fmt.Sprintf("hookflow_%s_%s_%s", latestVersion, osNames[runtime.GOOS], archNames[runtime.GOARCH])
	...
	if !opts.Yes {
		log.Infof("Update %s to %s? %s ", log.Cyan("hookflow"), log.Yellow(latestVersion), log.Gray("[Y/n]"))
	...
	hookflowExePath := opts.ExePath
	if realPath, serr := filepath.EvalSymlinks(hookflowExePath); serr == nil {
		hookflowExePath = realPath
	}

	destPath := hookflowExePath + "." + latestVersion
	defer func() {
	...
	backupPath := hookflowExePath + ".bak"
	defer func() {
	...
	log.Debugf("mv %s %s", hookflowExePath, backupPath)
	if err = os.Rename(hookflowExePath, backupPath); err != nil {
		return fmt.Errorf("failed to backup hookflow executable: %w", err)
	}

	log.Debugf("mv %s %s", destPath, hookflowExePath)
	err = os.Rename(destPath, hookflowExePath)
	if err != nil {
		log.Errorf("Failed to replace the hookflow executable: %s", err)
		if err = os.Rename(backupPath, hookflowExePath); err != nil {
	...
	log.Debugf("chmod +x %s", hookflowExePath)
	if err = os.Chmod(hookflowExePath, modExecutable); err != nil {
